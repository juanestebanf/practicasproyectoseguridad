...
class AlertDetailScreen extends StatelessWidget {
              ...
              child: Column(
                ...
                children: [
                  if (alert.reporteTexto != null && alert.reporteTexto!.isNotEmpty)
                    _buildInfoRow(Icons.description_rounded, "Notas del Operador", alert.reporteTexto!),
                  
                  if (alert.reporteTexto != null && alert.reporteTexto!.isNotEmpty && alert.reporteUrl != null)
                    _buildDivider(),
                  
                  if (alert.reporteUrl != null)
                    _buildInfoRow(Icons.picture_as_pdf_rounded, "Evidencia Digital", "Ver documento adjunto", isLink: true),
                    
                  if ((alert.reporteTexto == null || alert.reporteTexto!.isEmpty) && alert.reporteUrl == null)
                    const Center(
                      child: Padding(
                        padding: EdgeInsets.symmetric(vertical: 8),
                        child: Text("Sin documentación", style: TextStyle(color: Colors.grey, fontStyle: FontStyle.italic)),
                      ),
                    ),
                ],
              ),
            ...
            child: Column(
              ...
              children: [
                ...
                Text(
                  value,
                  style: TextStyle(
                    color: isLink ? ColoresApp.info : Colors.white,
  ...
  Color _getColorByType(AlertEventType tipo) {
    switch (tipo) {
      ...
      case AlertEventType.cambioEstado: return ColoresApp.info;
  ...
  Widget _buildStatusBadge(AlertStatus status) {
    ...
    switch (status) {
      ...
      case AlertStatus.autoridad_asignada: color = Colors.purple; text = "ASIGNADA"; break;
      case AlertStatus.esperando_aprobacion: color = Colors.deepOrange; text = "EN REVISIÓN"; break;
